...
function UpdateLec(props) {
    ...
    const fetchLec = async () => {
        try {
            const res = await axios.get(`http://localhost:5000/getLec/${id}`);
            const lecData = res.data;

            // Format the date properly for input type="date"
            const formattedDate = new Date(lecData.date).toISOString().split('T')[0];
    ...
    return (
      ...
      <div className='container'>
        <div className='header'>
          <div className='text'>Update Lecture</div>
